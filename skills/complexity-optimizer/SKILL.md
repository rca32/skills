...
This skill may make a locally authorized optimization, but it never claims or releases an issue, changes tracker state, commits, pushes, opens a pull request, merges, or publishes evidence. For issue-backed edits, do not change files until the outer `work-github-issue` workflow confirms a valid implementation lease for this session. When a validated hotspot shows that consolidating shallow modules or moving a public seam is necessary, use `codebase-design` to recommend the target module shape; private in-bounds structure needs no extra approval. Use `tdd` for the behavior-preserving implementation slice when available, and leave final review to `code-review`.
