...
- If a supplied planning document declares `kind: "spec-explainer"`, `normative: false`, or an equivalent warning, read only enough metadata to resolve `derived_from`. Use the authoritative spec for constraints and ignore the explainer prose when selecting or evaluating interfaces.
