...
class ConfigureRepositoryContractTest(unittest.TestCase):
    ...
    def test_render_uses_explicit_policy_values(self) -> None:
        ...
        self.assertIn("single-context fallback", normalized)
        self.assertIn("If only the target advanced", result.stdout)
        self.assertIn("integration-base OID", result.stdout)
        self.assertIn("`sha` head precondition", result.stdout)
        self.assertIn("pins the integration base is optional", result.stdout)
        self.assertIn("Observe existing GitHub Actions", result.stdout)
        self.assertIn("final behavior-affecting candidate", normalized)
        self.assertIn("rerun only the", normalized)
        self.assertIn("tested prior candidate, final candidate", normalized)
        self.assertIn("targeted validator run on the final candidate", normalized)
        self.assertIn("fresh candidate-scoped context boundary", normalized)
        self.assertIn("every check and review axis", normalized)
        self.assertIn("artifact, integration assumption", normalized)
        self.assertIn("carry-forward rationale", normalized)
        self.assertIn("explicit review-context identity", normalized)
        self.assertIn("every required verification gate", normalized)
        self.assertIn("integration/base assumptions", normalized)
        self.assertIn("final targeted-validator result", normalized)
        self.assertIn("required catalog companion `code-review`", normalized)
        self.assertIn("missing required package", normalized)
        self.assertIn("`blocked|handoff` state", normalized)
        self.assertIn("never report completed", normalized)
        self.assertIn("immutable review base", normalized)
        self.assertIn("pin the live integration-base OID", normalized)
        self.assertIn("immutable review-base OID and current integration-base OID", normalized)
        self.assertIn("If those bases differ", normalized)
        self.assertIn("structured evidence plus its handoff pointer", normalized)
        self.assertIn("effective tree/diff fingerprint", normalized)
        self.assertIn("pass that artifact to both review axes", normalized)
        self.assertIn("fail closed if the result cannot be reconstructed", normalized)
        self.assertIn("provider run/status URL", normalized)
        self.assertIn("observed head OID", normalized)
        self.assertIn("effective-integration construction method", normalized)
        self.assertIn("durable artifact pointer", normalized)
        ...
        self.assertNotIn("Run the repository-defined focused and full tests", result.stdout)
        self.assertNotIn("reconcile it back to open", result.stdout)
        self.assertIn("external approval", result.stdout)
