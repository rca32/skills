...
| `local-work` | Spec-fingerprint-bound local work set, sequential item execution checkpoints, concise local evidence, and final local convergence | Product-behavior authority, tracker or lease mutation, per-item branches/worktrees, commit, push, publication |
...
`documenting-work` is the single source of truth for where durable development documents, document sets, and controlled derived representations live. Before another skill writes a domain model, spec, spec explainer, decision map, local-work set, decision, research note, diagnosis, review, handoff, or evidence artifact, it must resolve:

1. persistence tier: conversation, tracker, repository, or artifact;
2. exactly one authoritative body for each meaning or declared document-set role;
...
Bundled controlled projections are explicit and narrow. A `spec-explainer` must remain separately loadable, declare `normative: false`, identify the source authority, bind to its exact body fingerprint, and remain excluded from ticket decomposition, module design, implementation, test derivation, quality bars, and Spec review authority. A `local-work` set must also be fingerprint-bound and non-normative; it may control local execution order, scope, progress, and concise evidence, but product behavior and review requirements come only from its source spec. A decision-map entry point is an index, while each child decision is the sole authority for that decision. Do not generalize any of these shapes to permit arbitrary duplicate bodies.

## Runtime prerequisites

GitHub issue workflows require Git, Python 3, an authenticated GitHub CLI, a canonical GitHub remote that matches the requested repository, permission to push the atomic lease refs, and a documented tracker-state/dependency mapping. Local analysis, implementation, tests, `decision-map`, `local-work`, and authorized repository-document edits do not inherit those GitHub prerequisites unless they are part of an issue-backed workflow. `local-work` deliberately uses the current worktree without per-item branches, worktrees, leases, or publication; select it only when sequential local ownership is sufficient. Resolve execution fields before editing, push/PR fields before publication, and merge authority, strategy, required checks, completion point, and cleanup before merge. When no higher authority defines cleanup, the bundled `work-github-issue` contract authorizes only a completed session to remove its own clean linked worktree and safely deletable local ticket branch after exact provenance, recovery, and ownership checks. It retains pre-existing or shared artifacts, non-complete workspaces, and remote branches; broader cleanup requires explicit user or consuming-repository authority. When repository instructions omit tracker semantics, use `work-github-issue/references/tracker-contract.md`.

## Invocation policy

- Keep `domain-modeling`, `codebase-design`, `tdd`, `diagnosing-bugs`, `complexity-optimizer`, `code-review`, `documenting-work`, and `work-github-issue` eligible for implicit discovery when their trigger descriptions are specific enough to avoid overlap.
