...
계획 단계에서는 `prepare-issue` 뒤, `to-spec` 앞에 조건부로 사용합니다. 설계 결정을 먼저 끝내고 `to-spec`이 그 결정을 명세로 고정하게 합니다.
...
- migration과 남은 위험
- 기존 권위로 고정됨, private 구현 재량, 승인 범위 안에서 위임됨, 명시적으로 수락됨, 또는 수락 대기 중이라는 상태
- 다음 담당 workflow
