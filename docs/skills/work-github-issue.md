...
저장소가 상시 publication 권한을 명시했다면 이미 결정된 내용을 사람에게 다시 묻지 않습니다. 로컬 테스트와 서로 독립적인 Standards·Spec 리뷰를 통과한 뒤 허가된 방식으로 PR을 병합하고, 원격 복구 가능성과 작업공간 안전성을 확인해 정리한 다음 evidence와 함께 이슈를 닫습니다.
...
소비 저장소에 이 계약이 없다면 사용자의 명시적인 저장소 정책 변경 요청으로 [표준 관리형 계약](../../skills/work-github-issue/references/repository-contract.md)을 `AGENTS.md`에 설치할 수 있습니다. 기본 문구는 GitHub Actions를 금지하고 로컬 검증을 사용하며, 독립 리뷰의 blocker/high 지적을 해결한 lease 소유 에이전트에게 `main` 대상 squash merge 권한을 부여합니다. 대상 branch와 merge 방식은 설치 명령에서 바꿀 수 있습니다.
...
정리는 lease 해제에 묶지 않습니다. 다른 세션이 같은 이슈를 가져가는 경합을 피하려고 최종 evidence·이슈 종료·release 전에 lease를 다시 확인한 상태에서 수행하고, 제거 결과 또는 보존 이유를 evidence에 기록합니다. 강제 worktree 제거, `git branch -D`, 광범위한 prune은 사용하지 않습니다. cleanup disposition이 정해지고 evidence가 readback된 뒤에만 완료 이슈를 닫습니다. release가 확인된 뒤에는 자동 정리를 새로 시작하지 않습니다.
