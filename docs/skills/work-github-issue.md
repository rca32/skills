...
저장소가 상시 publication 권한을 명시했다면 이미 결정된 내용을 사람에게 다시 묻지 않습니다. 로컬 테스트와 분리된 Standards·Spec 리뷰를 통과한 뒤 허가된 방식으로 PR을 병합하고, 원격 복구 가능성과 작업공간 안전성을 확인해 정리한 다음 evidence와 함께 이슈를 닫습니다. 독립 리뷰 환경이 없으면 한 컨텍스트에서 두 기준을 순서대로 검토해도 완료할 수 있습니다.
...
저장소에 별도 tracker 규칙이 없으면 사람이 바로 이해할 수 있는 한국어 라벨이 이미 있을 때 사용합니다. 없으면 같은 role을 나타내는 이슈 body marker를 사용하므로 라벨 설정이 작업 시작의 전제는 아닙니다.
...
자동화에서는 각각 `needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, `wontfix`라는 역할 키를 사용합니다. 기존 영문 라벨도 읽을 수 있지만, 새 상태는 기존 한국어 라벨 하나 또는 body marker 하나로 표현합니다.
...
최종 ticket commit을 만든 뒤 그 OID에서 로컬 검증을 실행하고, 작업 전 fixed point부터 해당 OID까지 `code-review`로 Standards와 Spec을 분리해 검토합니다. 독립 리뷰 환경이 있으면 결론을 공유하지 않고 병렬 또는 별도 컨텍스트로 진행하며, 없으면 한 컨텍스트에서 순서대로 수행하고 그 사실을 밝힙니다. 수정으로 OID가 바뀌면 변경된 동작을 다루는 검증과 리뷰를 새 OID에서 다시 실행합니다. [게시 절차](../../skills/work-github-issue/SKILL.md#4-review-and-publish-evidence)는 issue lease와 코드 게시 권한을 분리하고, commit, push, PR과 merge를 허가된 단계까지만 수행하게 합니다. 저장소가 에이전트 자동 병합을 상시 허가했다면 live PR head와 remote ticket ref가 검증·리뷰된 OID와 정확히 같을 때만 사람의 중복 승인을 기다리지 않고 지정한 방식으로 병합합니다. base만 바뀌면 effective merge diff와 위험 관련 검사를 다시 확인하며 전체 리뷰를 무조건 반복하지 않습니다.
...
정리는 lease 해제에 묶지 않습니다. 다른 세션이 같은 이슈를 가져가는 경합을 피하려고 최종 evidence·release 전에 lease를 다시 확인한 상태에서 수행하고, 제거 결과 또는 보존 이유를 evidence에 기록합니다. 강제 worktree 제거, `git branch -D`, 광범위한 prune은 사용하지 않습니다. merge의 closing keyword가 이슈를 먼저 닫아도 다시 열지 않고 cleanup과 evidence를 끝낸 뒤 release합니다. 아직 열려 있다면 그때 닫습니다. release가 확인된 뒤에는 자동 정리를 새로 시작하지 않습니다.
...
- 외부 쓰기 결과가 unknown이면 최대 60초·세 번의 읽기로 조정하고, 안전하게 absent로 확인된 create만 한 번 재시도합니다. 계속 unknown이면 복구 marker를 보존하고 lease 갱신을 멈춥니다.
- 사용자가 허용한 범위 밖의 commit, push, PR 생성 또는 merge를 하지 않습니다.
- 충돌이 없고 원격 기본 branch가 하나뿐이면 base와 PR target으로 쓸 수 있지만, merge나 remote 삭제 권한으로 해석하지 않습니다.
